...
A modern, secure NNTPS (Network News Transfer Protocol over TLS) reader built on the hyperpolymath verified container ecosystem.

== Overview

No-Nonsense NNTPS is a next-generation newsgroup reader designed for modern content formats while maintaining compatibility with traditional NNTP/NNTPS infrastructure. Built on formally verified components and the Vörðr security stack.

**Security-First:** Only supports NNTPS (secure). No insecure NNTP support by design.

**Tech Stack:**
- Elixir (orchestration, GenStateMachine for connection management)
- ReScript TEA + cadre-tea-router (UI)
- Svalinn (edge gateway & auth)
- Vörðr (container runtime)
- Cerro Torre (provenance verification)
- Selur (zero-copy IPC)
- Proven library (formal verification where applicable)
